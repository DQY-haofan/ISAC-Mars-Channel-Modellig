...
def compute_pareto_boundary_scalarized(system, tau_vis, S4, num_weights=101):
    ...
    # Use cosine-weighted sampling for denser endpoints
    t = np.linspace(0, 1, num_weights)
    lambdas = 0.5 * (1 - np.cos(np.pi * t))  # Denser at endpoints
    frontier_s = []
    frontier_r = []
    ...
    # Optional: Apply PCHIP smoothing for publication quality
    try:
        from scipy.interpolate import PchipInterpolator
        if len(s_sorted) > 3:  # Need at least 4 points for PCHIP
            # Remove duplicates for interpolation
            unique_idx = np.unique(s_sorted, return_index=True)[1]
            s_unique = s_sorted[unique_idx]
            r_unique = r_envelope[unique_idx]
            
            if len(s_unique) > 3:
                pchip = PchipInterpolator(s_unique, r_unique)
                s_smooth = np.linspace(s_unique.min(), s_unique.max(), 200)
                r_smooth = pchip(s_smooth)
                # Ensure monotonicity after interpolation
                r_smooth = np.maximum.accumulate(r_smooth[::-1])[::-1]
                return s_smooth, r_smooth
    except ImportError:
        pass  # Fall back to original if scipy not available
